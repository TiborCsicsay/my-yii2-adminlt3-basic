...
namespace app\modules\translatemanager\helpers;

use Yii;
use app\modules\translatemanager\services\Scanner;
use app\modules\translatemanager\bundles\TranslationPluginAsset;
...
class Language
{
     ...
     *      ],
     * ];
     * $result = \app\modules\translatemanager\helpers\Language::a($array, $params);
     * ~~~
     *
     * The result:
     *
    ...
    {
        return Yii::$app->session->has(\app\modules\translatemanager\Module::SESSION_KEY_ENABLE_TRANSLATE);
    }
    ...
    {
        $languageSources = \app\modules\translatemanager\models\LanguageSource::find()->where(['category' => $category])->all();
        ...
        if (empty($messages[$message])) {
            $languageSource = new \app\modules\translatemanager\models\LanguageSource();
    ...
    {
        $languageSources = \app\modules\translatemanager\models\LanguageSource::find()->select('category')->distinct()->all();
