...
'controllerMap' => [
    'migrate' => [
        ...
        'migrationNamespaces' => [
            'app\modules\translatemanager\migrations\namespaced',
        ],
    ],
],
```
...
'components' => [
    'translatemanager' => [
        'class' => 'app\modules\translatemanager\Component'
    ]
]
```
...
```php
\app\modules\translatemanager\helpers\Language::registerAssets();
```
...
```php
\app\modules\translatemanager\widgets\ToggleTranslate::widget();
```
...
```php
\app\modules\translatemanager\widgets\ToggleTranslate::widget([
 'position' => \app\modules\translatemanager\widgets\ToggleTranslate::POSITION_TOP_RIGHT,
 ...
 'frontendTranslationAsset' => 'app\modules\translatemanager\bundles\FrontendTranslationAsset',
 'frontendTranslationPluginAsset' => 'app\modules\translatemanager\bundles\FrontendTranslationPluginAsset',
]);
```
...
```php
use app\modules\translatemanager\helpers\Language as Lx;
...
**IMPORTANT: The app\modules\translatemanager\helpers\Language::t() (Lx::t()) function currently does not support the translation of HTMLattributes**
...
public function getStatuses() {
    return \app\modules\translatemanager\helpers\Language::a($this->_STATUSES);
}
...
public function getGenders() {
    return \app\modules\translatemanager\helpers\Language::a($this->_GENDERS, 'de-DE');
}
```
...
class Category extends \yii\db\ActiveRecord {
    ...
    public function behaviors()
    {
        return [
            [
                'class' => \app\modules\translatemanager\behaviors\TranslateBehavior::className(),
...
```php
'controllerMap' => [
    'translate' => \app\modules\translatemanager\commands\TranslatemanagerController::className()
],
```
